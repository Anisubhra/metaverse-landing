'use client';

import { motion } from 'framer-motion';
import { footerVariants } from '../utils/motion';
import { cn } from '../lib/utils';
import styles from '../styles';
import { socials } from '../constants';

const Footer = () => (
  <motion.footer
    variants={footerVariants}
    initial="hidden"
    whileInView="show"
    className={cn(styles.paddings, 'py-8 relative')}
  >
    <div className='footer-gradient' />

    <div className={cn(styles.innerWidth, 'mx-auto flex flex-col')}>
      <div className='flex items-center justify-between flex-wrap gap-5'>
        <h4 className='font-bold md:text-[64px] text-[44px] text-white'>
          Enter the Metaverse
        </h4>
        <button className='flex items-center h-fit py-4 px-6 bg-[#25618b] rounded-[32px] gap-[12px]'>
          <img src='/headset.svg' alt="headset" className='w-[24px] h-[24px] object-contain' />
          <span className='font-norma text-[16px] text-white'>ENTER METAVERSE</span>
        </button>
      </div>

      <div className='flex flex-col mt-[70px]'>
        <div className='mb-[50px] h-[2px] bg-white opacity-10' />

        <div className='flex items-center justify-between flex-wrap gap-4'>
          <h4 className='font-extrabold text-[24px] text-white'>HorizonX</h4>
          <p className='font-normal text-[14px] text-white opacity-50'>Copyright © HorizonX. All rights reserved.</p>
          <div className='flex gap-[30px]'>
            {
              socials.map((social) => (
                <img
                  src={social.url}
                  alt={social.name}
                  key={social.name}
                  title={social.name}
                  className='object-contain w-[24px] h-[24px] cursor-pointer'
                />
              ))
            }
          </div>
        </div>
      </div>
    </div>
  </motion.footer>
);

export default Footer;
